/**
 * Edge Dissolution Fragment Shader
 * Perlin noise-based erosion with edge glow
 */
...
uniform float u_dissolve_progress;
uniform float u_dissolve_noiseScale;
uniform float u_dissolve_edgeWidth;
uniform vec3 u_dissolve_edgeColor;
uniform sampler2D u_baseTexture; // Previous pass result

varying vec2 vUv;

// ============================================
// PERLIN NOISE (Ashima implementation)
// ============================================
...
void main() {
    ...
    // Skip already transparent areas
    ...
    if (diss.mask < 0.01) discard;

    // Add edge glow
    vec3 finalColor = mix(baseColor.rgb, u_dissolve_edgeColor, diss.edgeGlow);
    float finalAlpha = baseColor.a * diss.mask;
