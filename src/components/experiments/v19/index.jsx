...
/**
 * V19: Galaxy Simulator Experiment
 *
 * Visual Effect:
 ...
 *
 * Technical Details (from Galaxy Simulator PRD):
 * - Cube root distribution for uniform star density
 * - Logarithmic spirals: r = a × e^(b × θ)
 * - Blackbody temperature colors (O→B→A→F→G→K→M star types)
 * - Three-zone soft edge falloff (85% core, 85-100% transition, 100%+ decay)
 * - Kepler-inspired orbital motion: angular velocity ∝ 1/√radius
 * - Procedural dust lanes following spiral structure
 */
  ...
  return (
    <>
      <BaseExperimentShader
        ...
        title={`${CURRENT_ID.toUpperCase()}: ${current?.name || 'Galaxy'}`}
      />
      <NavOverlay>
